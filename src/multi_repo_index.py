#!/usr/bin/env python3
"""
Multi-Repository Index Builder
This script allows indexing multiple code repositories into the same Weaviate collection
with proper separation using metadata.
"""
import os
import logging
import sys
...
def split_by_function_or_class(file_path: str, repo_name: str = "unknown") -> List[TextNode]:
    """
    Split file content based on language-specific structures with repository metadata:
    - Python: by functions/classes (def, class)
    - Go: by functions (func)
    - Shell/Dockerfile/Makefile: by functions/sections
    Each TextNode contains the text and metadata (file_path, chunk_index, repo_name).
    """
...
def split_python_file(content: str, file_path: str, repo_name: str) -> List[TextNode]:
...
def split_go_file(content: str, file_path: str, repo_name: str) -> List[TextNode]:
...
def split_shell_file(content: str, file_path: str, repo_name: str) -> List[TextNode]:
    """Split shell script by functions."""
    import re
    
    # Pattern to match shell functions
    pattern = r'^(\w+)\s*\(\)\s*\{(?:[^{}]|\{[^{}]*\})*\}(?=\n^\w+\s*\(\)\s*\{|$)'
    matches = re.findall(pattern, content, re.MULTILINE)
...
def split_dockerfile(content: str, file_path: str, repo_name: str) -> List[TextNode]:
...
def split_makefile(content: str, file_path: str, repo_name: str) -> List[TextNode]:
...
def split_generic_file(content: str, file_path: str, repo_name: str) -> List[TextNode]:
...
def index_multiple_directories(directories: List[str], collection_name: str = None):
    """
    Index multiple directories into the same Weaviate collection with repository separation.
    
    Args:
        directories: List of directory paths to index
        collection_name: Name of the Weaviate collection (defaults to WEAVIATE_CLASS_NAME)
    """
    collection_name = collection_name or WEAVIATE_CLASS_NAME
    
    logger.info(f"Starting multi-directory index build process...")
    logger.info(f"Using device: {DEVICE}")
    logger.info(f"Weaviate endpoint: http://{WEAVIATE_HOST}:{WEAVIATE_PORT}")
    logger.info(f"Weaviate class name: {collection_name}")
    ...
    try:
        ...
        # Check if collection exists and warn user
        if client.collections.exists(collection_name):
            logger.warning(f"Collection '{collection_name}' already exists and will be appended to!")
            logger.info(f"New content will be added to existing collection without removing previous content.")
        else:
        ...
        for i, directory in enumerate(directories):
            ...
            logger.info(f"Found {len(all_code_files)} code files in directory: {directory}")
            
            if not all_code_files:
                logger.warning(f"No source files found in directory: {directory}")
                continue

            # Process all files and create nodes with repository metadata
            dir_nodes = []
            for j, file_path_obj in enumerate(all_code_files):
                file_path_str = str(file_path_obj)
                logger.info(f"Processing file {j+1}/{len(all_code_files)}: {file_path_str}")
                
                # Use our language-specific splitting function with repository name
                file_nodes = split_by_function_or_class(file_path_str, repo_name)
                dir_nodes.extend(file_nodes)

            logger.info(f"Generated {len(dir_nodes)} nodes from directory: {directory}")
            all_nodes.extend(dir_nodes)

        if not all_nodes:
            logger.warning("No nodes were generated from any directory.")
            return

        logger.info(f"Total nodes generated: {len(all_nodes)} from {len(directories)} directories")
        ...
        logger.info("Multi-directory index building process completed successfully.")
        logger.info(f"Added index with {len(all_nodes)} nodes to collection '{collection_name}'")
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python multi_repo_index.py <directory1> [directory2] [directory3] ...")
        print("Example: python multi_repo_index.py /path/to/repo1 /path/to/repo2 /path/to/repo3")
        sys.exit(1)
    
    directories = sys.argv[1:]
    index_multiple_directories(directories)
